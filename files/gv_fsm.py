...
class GV(AbstractFSM):
    ...
    transitions = [
        ...
        },
        {
            ...
            "source": "feedback",
            "dest": "ask_another_question",
            "conditions":"is_satisied"
        },
        {
            "trigger": "next",
            "source": "feedback",
            "dest": "register_query",
            "conditions":"is_not_satisied"
        },
    ]

    conditions = {"is_in_scope", "is_not_in_scope"}
    ...
    def on_enter_select_language(self):
        ...
        self.send_message(
            FSMOutput(
                message_data=MessageData(body="Hey there! Welcome to GV!"),
    ...
    def on_enter_ask_for_question(self):
        self.status = Status.WAIT_FOR_ME
        message="Proceed to ask your question regarding provident fund, etc"
...
You are an helpful assistant who helps with answering questions. Given the user's query, you should display the corresponding answer from the given chunk knowledge in the exact format. Do not make up or paraphrase any answers. Ensure that the output is presented in string format.
        ...
        self.send_message(FSMOutput(message_data=MessageData(body=result)))
        self.status = Status.MOVE_FORWARD
    ...
    def on_enter_register_query(self):
        ...
        self.send_message(
            FSMOutput(
                message_data=MessageData(body="Registering your query for review, will get back to you soon"),
            )
        )
        self.status = Status.MOVE_FORWARD

    def is_in_scope(self):
        return self.variables["query"]

    def is_not_in_scope(self):
        return not self.variables["query"]
